...
# camera = cv.VideoCapture("angryy.mp4")
# camera=cv.VideoCapture("5.mp4")
    ...
    while True:
        ...
        if results.multi_face_landmarks:
            ...
            line_ratio = lineRatio(
                frame, mesh_coords, RIGHT_EYE_LINE, LEFT_EYE_LINE)

            if (FRAME_COUNT <=48):
                emotion = ""
                # conditions for emotions
                if eyeRatio < 3.0 and line_ratio > 1.2:
                    cv.putText(frame, f'emotion is anger', (50, 50),
                    ...
                    cv.putText(frame, f'emotion is neutral', (50, 50),
                               cv.FONT_HERSHEY_TRIPLEX, 1.0, (255, 0, 0), thickness=2)
                    emotion = "neutral"
...
for i in emotion_data:
        if i["main-lead-emotion"] == "neutral":
            neutralsum+=1
        elif i["main-lead-emotion"]=="happy":
             happysum+=1
        elif i["main-lead-emotion"]=="angry":
             angrysum+=1
        else:
            surprisesum+=1
if happysum or angrysum or surprisesum or neutralsum:
            percentage = {
                "happy_mainlead_percentage": "{:.2f}".format((happysum/len(emotion_data))*100),
                "angry_mainlead_percentage": "{:.2f}".format((angrysum/len(emotion_data))*100),
                "surprise_mainlead_percentage": "{:.2f}".format((surprisesum/len(emotion_data))*100),
                "neutral_mainlead_percentage": "{:.2f}".format((neutralsum/len(emotion_data))*100)}
else:
            percentage = {
                "happy_mainlead_percentage": 0,
                "angry_mainlead_percentage": 0,
                "surprise_mainlead_percentage": 0,
                "neutral_mainlead_percentage": 0}

res = {}
res["emotions"] = emotion_data
